...
async function gameLoop() {
    ...
    while (step < MAX_STEPS && isRunning) {
        ...
        for (let cell of cells) {
            ...
            let alive = Array.from(neighbours).filter(n => document.querySelector(`[data-row="${n.split('-')[0]}"][data-col="${n.split('-')[1]}"]`).style.backgroundColor == 'black').length;            let markedForDeath = (alive >= 4 || alive <= 1);
...
//     const minimumGridSize = calculateMinimumSpace(JSON.stringify(gameState));
//     gameState.gridSize = minimumGridSize; // Update the grid size
...
function saveGameState() {
    ...
    const stateGrid = createGridFromMinimumState(JSON.stringify(gameState));
    const gameStateJSON = JSON.stringify(gameState);
    console.log('Saving game state:', gameStateJSON);
    stateGrid.dataset.gameState = gameStateJSON;
    savedStates.push(stateGrid);
    displaySavedGameStates();
    createGrid();
}
...
function loadGameState() {
    ...
    const innerDiv = selectedState.querySelector('div'); // Get the inner div
    const gameState = JSON.parse(innerDiv.dataset.gameState); // Get the game state from the inner div
    const gameBoard = document.getElementById('gameBoard');

    for (const cell of gameState.cells) {
        const gameCell = gameBoard.querySelector(`[data-row="${cell.row}"][data-col="${cell.col}"]`);
        gameCell.style.backgroundColor = cell.alive ? 'black' : 'white';
    }
}
